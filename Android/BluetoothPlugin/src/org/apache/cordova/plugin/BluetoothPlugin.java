		...
		if( m_bluetoothAdapter == null ) {
			...
			// Connect to a given device & uuid endpoint
				...
				try {
					...
					pw.println(msg);
					pluginResult = new PluginResult(PluginResult.Status.OK, String.valueOf("Enviado: "+msg));
					...
					try {
						...
						try {
							BluetoothSocket bluetoothSocket = m_bluetoothSockets.get(socketId);
							BufferedReader inputStream = new BufferedReader(new InputStreamReader(bluetoothSocket.getInputStream()));
							
							String linea = inputStream.readLine();
							
							Log.d( "BluetoothPlugin", "Buffer: " + String.valueOf(linea) );
							pluginResult = new PluginResult(PluginResult.Status.OK, String.valueOf(linea));
						}
						catch( Exception e ) {
							Log.e("BluetoothPlugin", e.toString() + " / " + e.getMessage() );
