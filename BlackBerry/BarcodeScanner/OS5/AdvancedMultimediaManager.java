 ...
 *
 * Minor modifications were made (denoted by BB: prefix in comment) to allow the
...
/**
 * <p>
 * Implementation suitable for JSR-234 phones which takes advantage of advanced
 * camera capability.
 * </p>
 *
 * @author Sean Owen
...
final class AdvancedMultimediaManager {
    ...
    // BB: Commenting out since not used.
//    private static final long FOCUS_TIME_MS = 750L;
    ...
    public void setFocus(Controllable player) {
        FocusControl focusControl = (FocusControl) getControl(player,
        ...
        if (focusControl != null) {
            try {
                ...
                if (focusControl.isAutoFocusSupported()) {
                    ...
                    // BB: Commented the following to allow auto focus to
                    // continually focus.
//                    try {
...
//                    focusControl.setFocus(FocusControl.AUTO_LOCK);
    ...
    public void setZoom(Controllable player) {
        ZoomControl zoomControl = (ZoomControl) getControl(player,
                "javax.microedition.amms.control.camera.ZoomControl");
        if (zoomControl != null) {
            // We zoom in if possible to encourage the viewer to take a snapshot
            // from a greater distance.
            // This is a crude way of dealing with the fact that many phone
            // cameras will not focus at a very close range.
            int maxZoom = zoomControl.getMaxOpticalZoom();
            if (maxZoom > NO_ZOOM) {
                zoomControl.setOpticalZoom(maxZoom > MAX_ZOOM ? MAX_ZOOM
                        : maxZoom);
            } else {
                int maxDigitalZoom = zoomControl.getMaxDigitalZoom();
                if (maxDigitalZoom > NO_ZOOM) {
                    zoomControl
                            .setDigitalZoom(maxDigitalZoom > MAX_ZOOM ? MAX_ZOOM
                                    : maxDigitalZoom);
                }
            }
        }
    ...
    public void setExposure(Controllable player) {
        ExposureControl exposureControl = (ExposureControl) getControl(player,
        ...
        if (exposureControl != null) {
            ...
            String[] supportedMeterings = exposureControl
    ...
    public void setFlash(Controllable player) {
        FlashControl flashControl = (FlashControl) getControl(player,
